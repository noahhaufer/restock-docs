...
3. The program has no instruction that moves an NFT out of that PDA, and its only Raydium call for the position
   collects fees with liquidity 0. Every transaction touching the position is on the explorer.
...
- the project-token side went to the indexer authority, and a later Jupiter swap by that key deposits the
  proceeds, in the stock, into the same holder vault.
...
1. Take a snapshot of the project token's holders at the publish slot. Exclude the pool's Raydium vault, apply the
